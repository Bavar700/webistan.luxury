			...
			<div class="relative z-10 mx-auto max-w-3xl px-6 text-center">
				...
				<h1 class="text-4xl sm:text-5xl font-serif font-medium text-gray-900 leading-tight mb-8" data-aos="fade-up" data-aos-delay="100">
					...
					if ( get_the_title() === 'Hello world!' ) {
						echo yaghnob_tr('hello_world_title');
					} else {
						the_title();
					}
					?>
		...
		<div class="py-16 sm:py-24">
			<div class="mx-auto max-w-3xl px-6">
				...
				<article class="prose lg:prose-lg prose-headings:font-serif prose-headings:font-medium prose-p:font-light prose-a:text-primary hover:prose-a:text-gray-900 transition-colors mx-auto text-gray-600">
					<?php 
					if ( get_the_title() === 'Hello world!' ) {
						echo '<p>' . yaghnob_tr('hello_world_content') . '</p>';
					} else {
						the_content();
					}
					?>
				...
				<div class="mt-16 pt-8 border-t border-gray-100">
					<div class="flex flex-col sm:flex-row items-center justify-between gap-6">
						<span class="text-xs font-bold uppercase tracking-widest text-gray-400"><?php echo yaghnob_tr('share_this'); ?></span>
						<div class="flex items-center gap-4">
							...
							<button onclick="navigator.clipboard.writeText('<?php echo get_permalink(); ?>'); alert('<?php echo yaghnob_tr("link_copied"); ?>');" class="p-3 rounded-full bg-ivory text-gray-600 hover:bg-primary hover:text-white transition-all duration-300 group" aria-label="Copy Link">
				...
				<div class="mt-16 pt-10 border-t border-gray-100 flex justify-between gap-8">
					<div class="w-1/2">
						<?php previous_post_link( '%link', '<span class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">' . yaghnob_tr('pagination_previous') . '</span><span class="font-serif text-lg text-gray-900 hover:text-primary transition-colors line-clamp-2">%title</span>' ); ?>
					</div>
					<div class="w-1/2 text-right">
						<?php next_post_link( '%link', '<span class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2">' . yaghnob_tr('pagination_next') . '</span><span class="font-serif text-lg text-gray-900 hover:text-primary transition-colors line-clamp-2">%title</span>' ); ?>
					</div>
				</div>
