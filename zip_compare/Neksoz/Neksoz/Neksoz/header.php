...
<header class="header">
    <div class="container header__inner">
        ...
        $nav_texts = [
            ...
            'tj' => ['Асосӣ', 'Хидмат', 'Дар бораи мо', 'Ахбор', 'Ҷойҳои корӣ', 'Тамос'],
        ...
        function nk_link($path, $lang) {
            $url = home_url($path);
            if ($lang === 'tj') {
                if ($path === '/news' || $path === 'news') return home_url('/news-tj?lang=tj');
                if ($path === '/about' || $path === 'about') return home_url('/about-tj?lang=tj');
                return add_query_arg('lang', 'tj', $url);
            }
            if ($lang === 'en') {
                if ($path === '/news' || $path === 'news') return home_url('/news-en?lang=en');
                if ($path === '/about' || $path === 'about') return home_url('/about-en?lang=en');
        ...
        <nav class="header__nav">
            <a href="<?php echo nk_link('/', $current_lang); ?>"><?php echo $texts[0]; ?></a>
            <a href="<?php echo nk_link('/#services', $current_lang); ?>"><?php echo $texts[1]; ?></a>
            <a href="<?php echo nk_link('/about', $current_lang); ?>"><?php echo $texts[2]; ?></a>
            <a href="<?php echo nk_link('/news', $current_lang); ?>"><?php echo $texts[3]; ?></a>
            <a href="<?php echo nk_link('/vacancies', $current_lang); ?>"><?php echo $texts[4]; ?></a>
            <a href="<?php echo nk_link('/contacts', $current_lang); ?>"><?php echo $texts[5]; ?></a>
        </nav>
        <div class="header__actions">
            ...
            <div class="lang-switcher">
                ...
                function nk_get_switcher_link($lang, $current_slug) {
                    // Logic to map RU slugs to TJ/EN counterparts
                    if ($lang === 'tj') {
                        if ($current_slug === 'news') return home_url('/news-tj?lang=tj');
                        if ($current_slug === 'about') return home_url('/about-tj?lang=tj');
                        return add_query_arg('lang', 'tj', home_url($wp->request));
                    }
                    if ($lang === 'en') {
                        if ($current_slug === 'news-tj' || $current_slug === 'news') return home_url('/news-en?lang=en');
                        if ($current_slug === 'about-tj' || $current_slug === 'about') return home_url('/about-en?lang=en');
                        return add_query_arg('lang', 'en', home_url($wp->request));
                    }
                    // Default to RU slugs
                    if ($current_slug === 'news-tj' || $current_slug === 'news-en') return home_url('/news?lang=ru');
                    if ($current_slug === 'about-tj' || $current_slug === 'about-en') return home_url('/about?lang=ru');
                    return add_query_arg('lang', 'ru', home_url($wp->request));
                }
                ?>
