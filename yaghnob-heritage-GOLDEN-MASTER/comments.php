...
<div id="comments" class="comments-area mt-16 pt-16 border-t border-gray-100 max-w-3xl mx-auto">
	...
	<?php if ( have_comments() ) : ?>
		<h2 class="text-2xl font-serif font-medium text-gray-900 mb-8">
			...
			if ( '1' === $comments_number ) {
				printf( yaghnob_tr( 'one_thought_on' ), '<span>' . get_the_title() . '</span>' );
			} else {
				printf(
					yaghnob_tr( 'thoughts_on' ),
							...
							<div class="flex-grow">
								<div class="bg-ivory p-6 rounded-2xl border border-gray-50">
									...
									<?php if ( '0' == $comment->comment_approved ) : ?>
										<p class="text-xs text-amber-600 mb-2"><?php echo yaghnob_tr( 'comment_awaiting_moderation' ); ?></p>
									<?php endif; ?>
		...
		the_comments_navigation(
			array(
				'prev_text' => '<span class="text-xs font-bold uppercase tracking-widest text-gray-500 hover:text-primary transition-colors">&larr; ' . yaghnob_tr('older_comments') . '</span>',
				'next_text' => '<span class="text-xs font-bold uppercase tracking-widest text-gray-500 hover:text-primary transition-colors">' . yaghnob_tr('newer_comments') . ' &rarr;</span>',
	...
	if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
		?>
		<p class="text-center text-gray-500 italic mt-8"><?php echo yaghnob_tr('comments_closed'); ?></p>
		<?php
	endif;

	$comment_form_args = array(
		'title_reply'          => '<span class="font-serif text-2xl text-gray-900">' . yaghnob_tr('leave_comment') . '</span>',
		'title_reply_before'   => '<h3 id="reply-title" class="comment-reply-title mb-6">',
		'title_reply_after'    => '</h3>',
		'label_submit'         => yaghnob_tr('post_comment'),
		...
		'comment_notes_before' => '<p class="comment-notes text-sm text-gray-500 mb-6">' . yaghnob_tr('email_note') . '</p>',
		'comment_field'        => '<div class="mb-6"><label for="comment" class="sr-only">' . yaghnob_tr('comment_noun') . '</label><textarea id="comment" name="comment" cols="45" rows="8" class="block w-full rounded-lg border-0 px-4 py-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6 resize-none transition-shadow duration-300" placeholder="' . yaghnob_tr('write_thoughts') . '" required></textarea></div>',
		'fields'               => array(
			'author' => '<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6"><div class="comment-form-author"><label for="author" class="sr-only">' . yaghnob_tr('name') . '</label><input id="author" name="author" type="text" value="" size="30" class="block w-full rounded-lg border-0 px-4 py-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6 transition-shadow duration-300" placeholder="' . yaghnob_tr('name') . ( $req ? '*' : '' ) . '" ' . ( $req ? 'required' : '' ) . ' /></div>',
			'email'  => '<div class="comment-form-email"><label for="email" class="sr-only">' . yaghnob_tr('email') . '</label><input id="email" name="email" type="email" value="" size="30" class="block w-full rounded-lg border-0 px-4 py-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary sm:text-sm sm:leading-6 transition-shadow duration-300" placeholder="' . yaghnob_tr('email') . ( $req ? '*' : '' ) . '" ' . ( $req ? 'required' : '' ) . ' /></div></div>',
			'url'    => '', // Remove website field to keep it simple
			'cookies' => '<p class="comment-form-cookies-consent mb-6 flex items-start gap-2"><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes" class="mt-1 h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" /> <label for="wp-comment-cookies-consent" class="text-sm text-gray-600">' . yaghnob_tr('save_info') . '</label></p>',
		),
	);
