...
class Stripe_Subscription extends \Voxel\Product_Types\Payment_Methods\Base_Payment_Method {
	...
	public function get_customer_actions(): array {
		...
		if ( in_array( $state['status'], [ 'incomplete', 'past_due', 'unpaid' ], true ) ) {
			...
			if ( \Voxel\Modules\Paid_Listings\get_order_package( $this->order ) ) {
				$actions[] = [
					'action' => 'active.upgrade',
					'label' => _x( 'Upgrade / Downgrade', 'order customer actions', 'voxel' ),
					'handler' => function() {
						return wp_send_json( [
							'success' => true,
							'redirect_to' => \Voxel\Modules\Paid_Listings\get_pricing_page_link( [
								'process' => 'upgrade',
								'order_id' => $this->order->get_id(),
								'redirect_to' => $this->order->get_link(),
							] ),
						] );
					},
				];
