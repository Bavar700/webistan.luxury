...
function navigateTo(page) {
    ...
    // Hide today-bar: show only on dashboard page
    const todayBar = document.querySelector('.today-bar');
    if (todayBar) {
        todayBar.classList.toggle('hidden', page !== 'dashboard');
    }
    ...
    if (child.withdrawals.length === 0) {
        ...
        histContainer.innerHTML = child.withdrawals.slice().reverse().map(w => {
            ...
            return `<div class="withdrawal-item">
                ...
                <span class="withdrawal-amount">-${w.amount} ${sym}</span>
            </div>`;
        }).join('');
    }
...
function showWithdrawModal() {
    ...
    let hasFunds = false;
    if (rt === 'money' && child.balance > 0) hasFunds = true;
    if (rt === 'stars' && (child.stars || 0) > 0) hasFunds = true;
    if (rt === 'both' && (child.balance > 0 || (child.stars || 0) > 0)) hasFunds = true;
    ...
    if (rt === 'both') {
        ...
        if (child.balance > 0) {
            typeSelect.innerHTML += `<option value="money">🪙 Тилло (${child.balance})</option>`;
        }
        if ((child.stars || 0) > 0) {
        ...
        typeSelect.innerHTML = rt === 'stars' 
            ...
            : `<option value="money">🪙 Тилло</option>`;
        typeSelect.value = rt;
    }
...
function submitWithdraw() {
    ...
    const successMsg = withdrawType === 'stars' 
        ? __('balance.withdraw_success_stars') 
        : __('balance.withdraw_success_money');
    showToast('🏦', `${amount} ${successMsg}`);
    renderBalance();
    updateUI();
}
