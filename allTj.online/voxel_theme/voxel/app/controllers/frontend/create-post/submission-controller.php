...
class Submission_Controller extends \Voxel\Controllers\Base_Controller {
	...
	protected function handle() {
		try {
			...
			// \Voxel\verify_nonce( $_POST['submit_nonce'] ?? '', 'vx_create_post' );
			...
			$visibility_context_result = $this->resolve_visibility_context_token( $_POST['visibility_context_token'] ?? null, [
			...
			$this->maybe_trigger_visibility_context_fallback( $visibility_context_result, $post_type, $post, false );
			$visibility_context = $visibility_context_result['context'] ?? null;
			$allowed_field_keys = $visibility_context_result['allowed_field_keys'] ?? null;
			...
			$fields = $this->filter_fields_by_allowed_keys( $fields, $allowed_field_keys );
			...
			foreach ( $fields as $field_key => $field ) {
				if ( isset( $hidden_steps[ $field->get_step() ] ) || ! $field->passes_visibility_rules( $visibility_context ) ) {
	...
	protected function handle_admin_mode() {
		try {
			...
			$this->maybe_trigger_visibility_context_fallback( $visibility_context_result, $post_type, $post, true );
			$visibility_context = $visibility_context_result['context'] ?? null;
			$allowed_field_keys = $visibility_context_result['allowed_field_keys'] ?? null;
	...
	protected function resolve_visibility_context_token( $token, array $bindings ): array {
		if ( is_string( $token ) ) {
		...
		return \Voxel\verify_visibility_context_token( trim( $token ), $bindings );
	}

	protected function maybe_trigger_visibility_context_fallback( array $result, \Voxel\Post_Type $post_type, ?\Voxel\Post $post, bool $is_admin_mode ): void {
		if ( ! empty( $result['valid'] ) ) {
			return;
		}

		do_action( 'voxel/create-post/visibility-context/fallback', [
	...
	protected function filter_fields_by_allowed_keys( array $fields, $allowed_field_keys ): array {
		...
		foreach ( $allowed_field_keys as $field_key ) {
			...
			if ( $field_key !== '' ) {
				$allowed_lookup[ 'key:'.$field_key ] = true;
			}
		}

		foreach ( $fields as $field_index => $field ) {
			$field_key = (string) $field->get_key();
			if ( ! isset( $allowed_lookup[ 'key:'.$field_key ] ) ) {
