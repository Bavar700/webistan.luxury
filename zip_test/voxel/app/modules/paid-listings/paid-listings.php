...
function get_settings_schema(): Data_Object {
	return Schema::Object( [
		...
		'auto_select_plan' => Schema::Bool()->default(false),
...
function get_pricing_page_link( array $args = [] ): string {
	$url = get_permalink( \Voxel\get( 'paid_listings.settings.templates.pricing' ) ) ?: home_url('/');

	return add_query_arg( array_filter( $args, static function( $value ) {
		return $value !== null && $value !== '';
	} ), $url );
}

function get_order_package( \Voxel\Order $order ): ?Module\Listing_Package {
...
function get_used_posts_from_limits( array $limits ): array {
	$posts = [];

	foreach ( $limits as $limit ) {
		foreach ( (array) ( $limit['usage']['posts'] ?? [] ) as $post ) {
			$post_id = absint( $post['id'] ?? 0 );
			$post_time = absint( $post['time'] ?? 0 );
			$post_type = sanitize_key( $post['type'] ?? '' );
			if ( ! $post_id || ! $post_type ) {
				continue;
			}

			$post_key = sprintf( '%d:%d', $post_id, $post_time );
			$posts[ $post_key ] = [
				'id' => $post_id,
				'type' => $post_type,
				'time' => $post_time,
...
function get_upgrade_post_allocation( array $current_limits, array $target_limits ): array {
	$current_posts = get_used_posts_from_limits( $current_limits );

	usort( $current_posts, static function( $left, $right ) {
		return [ $right['time'], $right['id'] ] <=> [ $left['time'], $left['id'] ];
	} );

	$remaining_posts = [];
	foreach ( $current_posts as $post ) {
		$remaining_posts[ sprintf( '%d:%d', $post['id'], $post['time'] ) ] = $post;
	}

	$kept_posts = [];
	foreach ( $target_limits as $limit ) {
		$candidates = array_values( array_filter( $remaining_posts, static function( $post ) use ( $limit ) {
			return in_array( $post['type'], (array) ( $limit['post_types'] ?? [] ), true );
		} ) );
		...
		foreach ( array_slice( $candidates, 0, max( 0, absint( $limit['total'] ?? 0 ) ) ) as $post ) {
			$post_key = sprintf( '%d:%d', $post['id'], $post['time'] );
			$kept_posts[ $post_key ] = $post;
			unset( $remaining_posts[ $post_key ] );
		}
	}

	return [
		'current_posts' => $current_posts,
		'kept_posts' => array_values( $kept_posts ),
		'expired_posts' => array_values( $remaining_posts ),
		'current_count' => count( $current_posts ),
		'kept_count' => count( $kept_posts ),
		'expired_count' => count( $remaining_posts ),
	];
}
