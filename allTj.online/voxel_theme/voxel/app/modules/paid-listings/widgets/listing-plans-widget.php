...
class Listing_Plans_Widget extends \Voxel\Widgets\Base_Widget {
	...
	protected function render( $instance = [] ) {
		...
		$process = \Voxel\from_list( $_GET['process'] ?? null, [ 'new', 'relist', 'claim', 'switch', 'upgrade' ], null );
		$submit_to = null;
		if ( $process === 'new' ) {
			...
			$upgrade_order = \Voxel\Order::find( [
				'id' => absint( $_GET['order_id'] ?? null ),
				'customer_id' => get_current_user_id(),
			] );
			$upgrade_package = $upgrade_order ? Module\get_order_package( $upgrade_order ) : null;
			$upgrade_payment_method = $upgrade_order ? $upgrade_order->get_payment_method() : null;

			if ( ! (
				$upgrade_order
				&& $upgrade_package
				&& $upgrade_package->get_plan()
				&& $upgrade_package->get_plan()->get_billing_mode() === 'subscription'
				&& $upgrade_payment_method
				&& $upgrade_payment_method->is_subscription_active()
			) ) {
				return null;
			}
		...
		$plans = [];
		$current_upgrade_amount = null;
		if ( $upgrade_package && ( $current_upgrade_plan = $upgrade_package->get_plan() ) ) {
			$current_upgrade_amount = $current_upgrade_plan->get_billing_discount_amount();
			if ( $current_upgrade_amount === null ) {
				$current_upgrade_amount = $current_upgrade_plan->get_billing_amount();
			}
		}
			...
			foreach ( $group['prices'] as $plan_key ) {
				try {
					...
					$billing = [
						...
						'mode' => $plan->get_billing_mode(),
						'effective_amount' => $plan->get_billing_discount_amount() !== null
					...
					$plans[] = [
						...
						'disable_repeat_purchase' => $plan->config('billing.disable_repeat_purchase'),
							'upgrade' => $upgrade_package ? array_merge(
								Module\get_upgrade_post_allocation( $upgrade_package->get_limits(), $plan->get_limits() ),
								[
									'is_current' => $upgrade_package->get_plan_key() === $plan->get_key(),
									'action_label' => (
										$current_upgrade_amount !== null
										&& $billing['effective_amount'] < $current_upgrade_amount
									) ? _x( 'Downgrade', 'listing plans', 'voxel' ) : _x( 'Upgrade', 'listing plans', 'voxel' ),
								]
							) : null,
					];
				} catch ( \Exception $e ) {
