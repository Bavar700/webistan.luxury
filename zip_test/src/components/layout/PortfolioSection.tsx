...
return ( <section id="portfolio" className="relative overflow-hidden bg-background">
...
<span className="text-[11px] md:text-[13px] font-display font-medium tracking-[1em] uppercase block mb-6 -mr-[1em] opacity-40" style={{ color: 'var(--calc-title-color)' }}> {t('subheading')
}
</span>
<div className="flex flex-col items-center">
<h2 className="text-4xl md:text-7xl font-display font-light tracking-[0.25em] leading-none uppercase -mr-[0.25em]" style={{ color: 'var(--calc-title-color)' }}> {t('legacies')
}
</h2>
...
className="group relative p-16 transition-all duration-700 hover:scale-[1.02] overflow-hidden rounded-lg border border-accent/10 bg-background" > {
...
<span className="text-[9px] md:text-[10px] font-display uppercase tracking-[0.6em] block -mr-[0.6em] font-medium opacity-40" style={{ color: 'var(--calc-title-color)' }}>Portfolio_Ref 0{i + 1}</span>
<h3 className="text-xl md:text-3xl font-display font-light group-hover:text-accent transition-all duration-1000 tracking-[0.2em] uppercase leading-none" style={{ color: 'var(--calc-title-color)' }}> {s.title.toLowerCase().includes('yaghnob.com') ? ( <a href="https://yaghnob.com" target="_blank" rel="noopener noreferrer" className="hover:underline decoration-accent/30 underline-offset-8" > {s.title
}
</a> ) : ( s.title )
}
</h3>
</div>
<p className="font-sans text-base leading-relaxed font-light mt-8 max-w-sm opacity-60" style={{ color: 'var(--calc-title-color)' }}> {s.desc
}
</p>
</div>
</motion.div> ))
