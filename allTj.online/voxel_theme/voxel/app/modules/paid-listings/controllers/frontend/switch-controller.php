...
class Switch_Controller extends \Voxel\Controllers\Base_Controller {
	...
	protected function hooks() {
		...
		$this->on( 'voxel_ajax_paid_listings.upgrade_listing_plan', '@upgrade_listing_plan' );
	...
	protected function upgrade_listing_plan() {
		try {
			\Voxel\verify_nonce( $_REQUEST['_wpnonce'] ?? '', 'vx_upgrade_listing_plan' );
			...
			if ( ! (
				...
				throw new \Exception( __( 'No subscription plan available.', 'voxel' ) );
			}

			return wp_send_json( [
				'success' => true,
				'redirect_to' => Module\get_pricing_page_link( [
					'process' => 'upgrade',
