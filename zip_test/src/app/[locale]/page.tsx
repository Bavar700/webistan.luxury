...
const tCommon = await getTranslations('Common');
  ...
  return (
    <main className="relative flex flex-col bg-background selection:bg-accent/10 selection:text-foreground">
      ...
      {/* THEME-AWARE BACKGROUND FROM PROMO ONWARDS */}
      <div className="bg-background flex flex-col px-6 md:px-12 lg:px-20">
        {/* Calculator Block */}
        <ProjectCalculator />
        ...
        {/* Ultra Minimal, Compact Footer (Theme-Aware) */}
        <footer className="py-12 border-t-[0.5px] border-accent/20 bg-background">
          <div className="container mx-auto px-6 max-w-7xl flex flex-col md:flex-row justify-between items-center gap-4">
            <Link href="/" className="flex items-center gap-2 group">
              ...
              <div className="text-[10px] md:text-[11px] tracking-[0.2em] font-display font-medium uppercase group-hover:text-accent transition-colors" style={{ color: 'var(--calc-title-color)' }}>
                WEBISTAN<span className="text-accent">.LUXURY</span>
              </div>
            </Link>

            <div className="flex flex-col md:flex-row items-center gap-3 md:gap-8">
              <a href="mailto:info@webistan.luxury" className="text-[9px] font-display uppercase tracking-[0.15em] hover:text-accent transition-colors" style={{ color: 'var(--calc-desc-color)' }}>info@webistan.luxury</a>
              <span className="text-[9px] font-display uppercase tracking-[0.15em]" style={{ color: 'var(--calc-desc-color)' }}>
