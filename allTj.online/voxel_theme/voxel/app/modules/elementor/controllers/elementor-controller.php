...
class Elementor_Controller extends \Voxel\Controllers\Base_Controller {
	...
	protected function hooks() {
		$this->register_atomic_vx();

		$this->on( 'elementor/widgets/register', '@register_widgets', 1000 );
		// Run late to ensure Voxel document labels/classes win over external providers.
		$this->on( 'elementor/documents/register', '@register_custom_document_types', 9999 );
		...
		$this->on( 'voxel_ajax_elementor.render_vx_tag', '@render_vx_tag_in_editor', 100, 2 );
	...
	protected function register_atomic_vx() {
		if ( ! class_exists( '\Elementor\Modules\AtomicWidgets\PropTypes\Base\Plain_Prop_Type' ) ) {
			return;
		}

		add_action( 'elementor/atomic-widgets/settings/transformers/register', function ( $transformers ) {
			$transformers->register( 'vx', new \Voxel\Modules\Elementor\Atomic_Vx\Vx_Transformer() );
		}, 100 );

		add_filter( 'elementor/atomic-widgets/props-schema', function ( array $schema ) {
			$schema = \Voxel\Modules\Elementor\Atomic_Vx\Vx_Prop_Types_Mapping::make()->get_extended_schema( $schema );
			$schema['_vx_visibility'] = \Voxel\Modules\Elementor\Atomic_Vx\Vx_Visibility_Prop_Type::make();
			$schema['_vx_loop'] = \Voxel\Modules\Elementor\Atomic_Vx\Vx_Loop_Prop_Type::make();
			$schema['_vx_dynamic_css'] = \Voxel\Modules\Elementor\Atomic_Vx\Vx_Dynamic_Css_Prop_Type::make();
			return $schema;
		}, 100 );
		...
		add_filter( 'elementor/atomic-widgets/controls', function ( array $controls, $element ) {
			$controls[] = \Elementor\Modules\AtomicWidgets\Controls\Section::make()
				->set_label( 'Visibility rules (VX)' )
				->set_items( [
					\Voxel\Modules\Elementor\Atomic_Vx\Vx_Visibility_Control::bind_to( '_vx_visibility' )
						->set_label( 'Visibility' ),
				] );

			$controls[] = \Elementor\Modules\AtomicWidgets\Controls\Section::make()
				->set_label( 'Loop item (VX)' )
				->set_items( [
					\Voxel\Modules\Elementor\Atomic_Vx\Vx_Loop_Control::bind_to( '_vx_loop' )
						->set_label( 'Loop element' ),
				] );

			$controls[] = \Elementor\Modules\AtomicWidgets\Controls\Section::make()
				->set_label( 'Dynamic styles (VX)' )
				->set_items( [
					\Voxel\Modules\Elementor\Atomic_Vx\Vx_Dynamic_Css_Control::bind_to( '_vx_dynamic_css' )
						->set_label( 'Dynamic CSS' ),
				] );
	...
	protected function render_vx_tag_in_editor() {
		try {
			$document_id = absint( $_REQUEST['post_id'] ?? 0 );
			...
			if ( ! $document_id && ! current_user_can( 'edit_posts' ) ) {
				throw new \Exception( __( 'Permission denied.', 'voxel-backend' ) );
			}

			$tag = wp_unslash( $_REQUEST['tag'] ?? '' );
			if ( ! is_string( $tag ) ) {
				$tag = '';
			}

			if ( $document_id ) {
				\Voxel\set_current_post( \Voxel\get_post_for_preview( $document_id ) );
				\Voxel\set_current_term( $this->_get_current_term_in_editor( $document_id ) );
			}

			$rendered = $tag === '' ? '' : \Voxel\render( $tag );

			return wp_send_json( [
				'success' => true,
				'rendered' => is_scalar( $rendered ) ? (string) $rendered : '',
			] );
		} catch ( \Exception $e ) {
			return wp_send_json( [
				'success' => false,
		...
		foreach ( $templates as $i => $template ) {
			if ( $template === 'navigator' ) {
				...
				$content = str_replace( '<span class="elementor-navigator__element__title__text">{{{ title }}}</span>', <<<HTML
					...
					<span onclick="_vx_nav_open_visibility(this)" {{ obj.settings?._vx_visibility ? 'data-vx-atomic' : '' }} class="elementor-navigator__element__vx_visibility {{ (Array.isArray( obj.settings?._voxel_visibility_rules ) && obj.settings._voxel_visibility_rules.length) || (obj.settings?._vx_visibility && Array.isArray(obj.settings._vx_visibility.value?.rules) && obj.settings._vx_visibility.value.rules.length) ? '' : 'hidden' }} _vx-el-nav-visibility-{{ obj.id }}">
						<i class="icon-element eicon-flow"></i>
					</span>
					<span onclick="_vx_nav_open_loop(this)" {{ obj.settings?._vx_loop ? 'data-vx-atomic' : '' }} class="elementor-navigator__element__vx_loops {{ (typeof obj.settings?._voxel_loop === 'string' && obj.settings._voxel_loop.length) || (obj.settings?._vx_loop && typeof obj.settings._vx_loop.value?.tag === 'string' && obj.settings._vx_loop.value.tag.length) ? '' : 'hidden' }} _vx-el-nav-loops-{{ obj.id }}">
	...
	protected function register_custom_document_types( $documents_manager ) {
		$shared_document_types = [
			'header' => \Voxel\Modules\Elementor\Documents\Header_Document::class,
			'footer' => \Voxel\Modules\Elementor\Documents\Footer_Document::class,
			'archive' => \Voxel\Modules\Elementor\Documents\Archive_Document::class,
			'single-post' => \Voxel\Modules\Elementor\Documents\Single_Post_Document::class,
		];

		foreach ( $shared_document_types as $type => $class_name ) {
			if ( $this->should_register_voxel_document_type( $documents_manager, $type, $class_name ) ) {
		...
		$documents_manager->register_document_type(
			'card',
			\Voxel\Modules\Elementor\Documents\Card_Document::class
		);

		$documents_manager->register_document_type(
			'single-term',
			\Voxel\Modules\Elementor\Documents\Single_Term_Document::class
		);
	}

	protected function should_register_voxel_document_type( $documents_manager, string $type, string $class_name ): bool {
		$registered_class = $documents_manager->get_document_type( $type, false );
		if ( ! is_string( $registered_class ) || $registered_class === '' ) {
			return true;
		}

		$registered_class = ltrim( $registered_class, '\\' );
		...
		return strpos( $registered_class, 'Voxel\\Modules\\Elementor\\Documents\\' ) === 0;
	}
