...
function normalize_search_results_delivery_mode( $delivery_mode ): string {
	if ( is_string( $delivery_mode ) ) {
		$delivery_mode = strtolower( trim( $delivery_mode ) );
		if ( in_array( $delivery_mode, [ 'ajax', 'ssr' ], true ) ) {
			return $delivery_mode;
		}
	}

	return wp_doing_ajax() ? 'ajax' : 'ssr';
}

function should_capture_search_result_scripts( string $delivery_mode ): bool {
	return $delivery_mode === 'ajax';
}
...
function get_search_results( $request, $options = [] ) {
	...
	$delivery_mode = \Voxel\normalize_search_results_delivery_mode( $options['delivery_mode'] ?? null );
		...
		if ( $options['render'] === 'markers' ) {
			...
			if ( $GLOBALS['vx_preview_card_level'] > 1 ) {
				...
				foreach ( $results['ids'] as $i => $post_id ) {
					...
					$rendered_template = \Voxel\render_template_with_context( $template_id, [
						'mode' => 'fragment',
						'print_styles' => false,
						'defer_styles' => true,
				...
				$new_style_handles = \Voxel\get_nested_style_handles_since( $styles_before_render );
				$results['styles'] = \Voxel\get_printed_style_markup( $new_style_handles, true );

				if ( \Voxel\should_capture_search_result_scripts( $delivery_mode ) ) {
					ob_start();
...
function _post_get_marker( $post ) {
	...
	if ( $marker_type === 'text' ) {
		...
		if ( $image_field && in_array( $image_field->get_type(), [ 'image', 'profile-avatar' ], true ) ) {
