...
<div className="group/promo w-screen relative left-1/2 -translate-x-1/2 py-[120px] overflow-hidden" style={{backgroundColor: '#000000'}}>
  <div className="max-w-6xl mx-auto relative border-[0.5px] border-accent/20 hover:border-accent/80 transition-colors duration-700 bg-black/40 backdrop-blur-sm px-6 py-32 md:px-12 text-center flex flex-col items-center justify-center">
    ...
    {/* Architectural Ribbon Anchor - Now inside the border container */}
...
<div className="relative">
  <LuxuryButton onClick={() => setIsDropdownOpen(!isDropdownOpen)}
    ...
    <div className="flex items-center justify-between w-full px-10">
      ...
      <ChevronDown className={`transition-transform duration-700 flex-shrink-0 ${isDropdownOpen ? 'rotate-180' : ''}`}
  ...
  <AnimatePresence> 
    {isDropdownOpen && ( 
      <motion.div 
        ...
        exit={{ opacity: 0, y: 10 }}
        className="absolute top-full left-0 w-full mt-2 border-none overflow-hidden shadow-2xl z-[60] bg-[#111111]"
      > 
        {types.map((type) => ( 
          <button 
            key={type.id}
            ...
            className="w-full text-left p-3 hover:bg-white/[0.03] transition-all duration-500 flex items-center justify-between group/item"
          >
...
<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4"> {finalGridItems.map((item, idx) => ( <button key={`${item.type}-${item.id}-${idx}`
}
onClick={() => {
...
<div className="flex-1 relative group/input">
            <input 
              ...
              className="w-full px-6 py-5 text-[10px] md:text-[11px] uppercase tracking-[0.3em] text-white/50 focus:outline-none placeholder:text-white/30 transition-all font-light bg-btn-bg" 
            />
...
<div className="flex gap-6"> {(['monthly', 'yearly'] as BillingCycle[]).map((cycle) => ( <button key={cycle
}
onClick={() => setBillingCycle(cycle)
}
className={`group relative w-40 md:w-48 h-[54px] md:h-[60px] transition-all duration-700 ${billingCycle === cycle ? 'bg-accent/[0.05]' : 'bg-btn-bg backdrop-blur-sm'}`}
>
...
].map((item) => ( <button key={item.level
}
...
id: 'velocity' as const, cost: '600 TJS' }, ].map((addon) => {
return ( <button key={addon.id
}
...
className="text-[10px] md:text-[11px] uppercase tracking-widest text-accent/60 hover:text-accent border-b border-accent/20 hover:border-accent transition-colors pb-1"> {t('details_drawer_trigger')
}
</button>
</div>
...
<div className="text-5xl md:text-9xl font-display font-extralight text-accent tracking-tighter flex items-center justify-center gap-4"> {originalPrice.toLocaleString()
}
...
<AnimatePresence> {isDrawerOpen && ( <div className="fixed inset-0 z-[100] flex justify-end">
<motion.div initial={{
opacity: 0 }
}
animate={{
...
className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setIsDrawerOpen(false)
}
/>
<motion.div initial={{
x: '100%' }
...
className="relative w-full max-w-md h-full bg-background backdrop-blur-sm border-l-[0.5px] border-accent/20 overflow-y-auto" >
