...
import { Cpu, Globe, Layout, ShoppingCart, ArrowUpRight } from 'lucide-react';
...
export const ServicesSection = () => {
    ...
    return (
        <section id="services" className="py-[125px] bg-background relative overflow-hidden scroll-mt-32">
            <div className="container mx-auto px-6 relative z-10 max-w-7xl">
                ...
                {/* Cards grid — fixed: gap-[40px] on mobile, gap-[100px] on desktop */}
                <motion.div
                    variants={containerVariants}
                    initial="hidden"
                    whileInView="visible"
                    viewport={{ once: true, amount: 0.1 }}
                    className="grid grid-cols-1 md:grid-cols-2 gap-[40px] md:gap-[100px] max-w-7xl mx-auto"
                >
                    {services.map((s, i) => (
                        <motion.div
                            key={i}
                        ...
                        >
                            {/* Shimmer overlay */}
                            <div
                                className="absolute inset-0 w-full h-full bg-gradient-to-br from-transparent via-accent/[0.03] to-transparent -translate-x-full group-hover:translate-x-full"
                            ...
                            <div className="relative z-10 flex justify-between items-start mb-24">
                                ...
                                <div className="w-10 h-10 flex items-center justify-center text-foreground/50 group-hover:text-accent group-hover:bg-accent/[0.03] transition-all duration-1000">
                            ...
                            <div className="relative z-10 text-center flex flex-col items-center">
                                <div className="space-y-4 w-full">
                                    <span className="text-[11px] md:text-[10px] font-display uppercase tracking-[0.6em] text-accent/80 block -mr-[0.6em] font-bold animate-pulse">
                                        {t('layer_label')}0{i + 1}
                                    </span>
                                    <h3 className="text-base md:text-2xl font-display font-light group-hover:text-accent transition-all duration-1000 tracking-[0.1em] uppercase leading-relaxed md:leading-tight pb-2">
                                        {t(`items.${s.key}.title`)}
                                    </h3>
                                </div>
                                <p className="font-sans text-base text-foreground/55 leading-relaxed font-light mt-8 max-w-sm">
                                    {t(`items.${s.key}.desc`)}
                                </p>
                            </div>
                        </motion.div>
