...
class Paid_Listings_Controller extends \Voxel\Controllers\Base_Controller {
	...
	protected function hooks() {
		...
		$this->filter( 'voxel/advanced-list/actions', '@register_upgrade_action' );
		$this->on( 'voxel/advanced-list/action:upgrade_listing_plan', '@render_upgrade_action', 10, 2 );
	...
	protected function register_upgrade_action( $actions ) {
		$actions['upgrade_listing_plan'] = __( 'Upgrade listing plan', 'voxel-elementor' );
		return $actions;
	}

	protected function render_upgrade_action( $widget, $action ) {
		require locate_template( 'app/modules/paid-listings/templates/frontend/upgrade-listing-plan-action.php' );
	}
	...
	protected function handle_post_deletion( $post_id ) {
		...
		if ( ! in_array( $package->order->get_status(), [ 'sub_active', 'sub_trialing', 'completed' ], true ) ) {
			return;
		}
