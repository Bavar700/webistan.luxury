...
class Create_Post extends Base_Widget {
	...
	protected function render( $instance = [] ) {
		...
		$config = [
			...
			'submit_nonce' => wp_create_nonce( 'vx_create_post' ),
		];
		...
		$context_ttl = absint( apply_filters(
			'voxel/create-post/visibility-context-ttl',
			6 * HOUR_IN_SECONDS,
			$post_type,
			$post,
			$is_admin_mode
		...
		$visibility_context = \Voxel\issue_visibility_context_token( [
			'uid' => absint( get_current_user_id() ),
			'blog_id' => absint( get_current_blog_id() ),
			'post_type' => $post_type->get_key(),
			'post_id' => $post ? $post->get_id() : null,
			'allowed_field_keys' => array_keys( $config['fields'] ),
			'context' => \Voxel\create_visibility_context_snapshot(),
		], $context_ttl );

		$config['visibility_context_token'] = $visibility_context['token'];
		$config['visibility_context_expires'] = $visibility_context['payload']['exp'];
