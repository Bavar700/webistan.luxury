...
require_once locate_template( 'app/utils/visibility-context-utils.php' );
...
require_once locate_template( 'app/utils/dm-utils.php' );
...
function currency_format( $price, $currency, $amount_is_in_cents = true, $always_show_fraction = false ) {
	...
	$custom_format = apply_filters( 'voxel/currency_format', null, $price, $currency, $always_show_fraction );
	...
	if ( ! $always_show_fraction && intval( $price ) == $price ) {
...
function evaluate_visibility_rules( $rules, ?array $context = null ): bool {
	return \Voxel\with_visibility_context( $context, function() use ( $rules ) {
...
function enqueue_maps() {
	if ( \Voxel\get('settings.maps.provider') === 'mapbox' && ! \Voxel\is_preview_mode() ) {
		...
		wp_enqueue_script( 'leaflet-gesture-handling' );
