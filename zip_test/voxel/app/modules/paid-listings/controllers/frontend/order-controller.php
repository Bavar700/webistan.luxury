...
class Order_Controller extends \Voxel\Controllers\Base_Controller {
	...
	protected function order_updated( $order ) {
		...
		$this->maybe_handle_subscription_upgrade( $order );
	...
	protected function maybe_handle_subscription_upgrade( \Voxel\Order $order ): void {
		$upgrade = $order->get_details( 'checkout.listing_plan_upgrade' );
		if ( ! is_array( $upgrade ) || ! empty( $upgrade['handled'] ) ) {
			return;
		}

		$package = Module\get_order_package( $order );
		if ( ! $package ) {
			return;
		}

		if ( $package->get_id() === absint( $upgrade['source_package_id'] ?? 0 ) ) {
			return;
		}

		$allocation = Module\get_upgrade_post_allocation(
			(array) ( $upgrade['old_limits'] ?? [] ),
			$package->get_limits()
		);

		$this->apply_upgrade_allocation( $package, $allocation );

		$order->set_details( 'checkout.listing_plan_upgrade', array_merge( $upgrade, [
			'handled' => true,
			'package_id' => $package->get_id(),
			'expired_count' => $allocation['expired_count'],
		] ) );
		$order->save( true );
	}

	protected function apply_upgrade_allocation( Module\Listing_Package $package, array $allocation ): void {
		foreach ( $allocation['kept_posts'] ?? [] as $post_config ) {
			$post = \Voxel\Post::get( $post_config['id'] ?? null );
			...
			try {
				...
				$this->expire_upgraded_listing( $post );
			}
		}

		foreach ( $allocation['expired_posts'] ?? [] as $post_config ) {
			$post = \Voxel\Post::get( $post_config['id'] ?? null );
			if ( $post ) {
				$this->expire_upgraded_listing( $post );
			}
	...
	protected function expire_upgraded_listing( \Voxel\Post $post ): void {
		if ( get_post_status( $post->get_id() ) !== 'expired' ) {
			wp_update_post( [
				'ID' => $post->get_id(),
				'post_status' => 'expired',
			] );
		...
		delete_post_meta( $post->get_id(), 'voxel:plan_reactivate_status' );
