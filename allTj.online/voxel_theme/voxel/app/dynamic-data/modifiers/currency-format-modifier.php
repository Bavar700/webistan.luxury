...
class Currency_Format_Modifier extends Base_Modifier {
	...
	protected function define_args(): void {
		$this->define_arg( [
			...
			'choices' => [ 'default' => 'Default platform currency' ] + \Voxel\Utils\Currency_List::all(),
		] );
		...
		$this->define_arg( [
			'type' => 'select',
			'label' => _x( 'Show decimals for whole amounts', 'modifiers', 'voxel-backend' ),
			'description' => _x( 'When Yes, whole amounts use the normal currency fraction rules (e.g. 16.00). Zero-decimal currencies are unchanged.', 'modifiers', 'voxel-backend' ),
			'choices' => [ '' => 'No', '1' => 'Yes' ],
		] );
	}
	...
	public function apply( string $value ) {
		...
		$always_show_fraction = !! $this->get_arg( 2 );

		return \Voxel\currency_format( $value, $currency, $amount_is_in_cents, $always_show_fraction );
	}
