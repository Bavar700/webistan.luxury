...
class Settings_Controller extends \Voxel\Controllers\Base_Controller {
	...
	protected function hooks() {
		...
		$this->on( 'admin_post_voxel_sync_template_data', '@sync_template_data' );
		$this->on( 'after_switch_theme', '@trigger_template_sync' );
	}
	...
	protected function trigger_template_sync() {
		do_action( 'voxel/templates/synchronize' );
	}

	protected function sync_template_data() {
		check_admin_referer( 'voxel_sync_template_data' );
		if ( ! current_user_can( 'manage_options' ) ) {
			die;
		}
		...
		return Schema::Object( apply_filters( 'voxel/global-settings/register', [
			...
			'messages' => Schema::Object( [
				...
				'rate_limit' => Schema::Object( [
					'time_between' => Schema::Int()->min(0)->default(1),
					'hourly_limit' => Schema::Int()->min(0)->default(50),
					'daily_limit' => Schema::Int()->min(0)->default(200),
				] ),
				'new_chats_limit' => Schema::Object( [
					'daily_limit' => Schema::Int()->min(0)->default(10),
				] ),
				'min_account_age' => Schema::Int()->min(0)->default(0),
				'files' => Schema::Object( [
					'enabled' => Schema::Bool()->default(true),
