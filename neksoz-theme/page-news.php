<?php
/**
 * Template Name: ÐÐ¾Ð²Ð¾ÑÑ‚Ð¸
 */
get_header();
?>

<main class="site-main">

    <!-- â•â•â•â•â•â•â•â•â•â•â• CINEMATIC HERO â•â•â•â•â•â•â•â•â•â•â• -->
    <section class="hero hero--internal">
        <div class="hero__geo"></div>
        <div class="hero__grid-pattern"></div>
        <div class="hero__accent-line"></div>
        <div class="hero__accent-line-2"></div>

        <div class="container hero__container" style="position:relative;z-index:2;">
            <div class="hero__content">
                <div class="hero__badge">ÐŸÑ€ÐµÑÑ-Ñ†ÐµÐ½Ñ‚Ñ€</div>
                <h1 class="hero__title">
                    Ð“Ð»Ð°Ð²Ð½Ñ‹Ðµ<br>
                    Ð½Ð¾Ð²Ð¾ÑÑ‚Ð¸ Ð¸ ÑÐ¾Ð±Ñ‹Ñ‚Ð¸Ñ<br>
                    <span class="text-gradient">Ð² Neksoz</span>
                </h1>
                <p class="hero__desc">
                    ÐÐºÑ‚ÑƒÐ°Ð»ÑŒÐ½Ñ‹Ðµ Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸Ñ Ð² Ð·Ð°ÐºÐ¾Ð½Ð¾Ð´Ð°Ñ‚ÐµÐ»ÑŒÑÑ‚Ð²Ðµ, ÑÐ¾Ð±Ñ‹Ñ‚Ð¸Ñ ÐºÐ¾Ð¼Ð¿Ð°Ð½Ð¸Ð¸ Ð¸ ÑÐºÑÐ¿ÐµÑ€Ñ‚Ð½Ñ‹Ðµ Ð¼Ð½ÐµÐ½Ð¸Ñ.
                </p>
            </div>
        </div>
    </section>

    <!-- â•â•â•â•â•â•â•â•â•â•â• NEWS LIST â€” EDITORIAL DESIGN â•â•â•â•â•â•â•â•â•â•â• -->
    <div class="section section--gray" style="padding: 120px 0;">
        <div class="container">
            <!-- Section Header -->
            <div class="section__header" style="margin-bottom: 60px;">
                <div class="section__label">ÐÑ€Ñ…Ð¸Ð² Ð¿ÑƒÐ±Ð»Ð¸ÐºÐ°Ñ†Ð¸Ð¹</div>
                <h2 class="section__title">Ð¥Ñ€Ð¾Ð½Ð¸ÐºÐ° Ð¿Ñ€Ð¾Ð³Ñ€ÐµÑÑÐ° Ð¸ Ð¼Ð½ÐµÐ½Ð¸Ñ ÑÐºÑÐ¿ÐµÑ€Ñ‚Ð¾Ð²</h2>
            </div>

            <div class="news-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(380px, 1fr)); gap: 50px;">
                
                <!-- Item 1: Law/Audit -->
                <article class="service-card fade-up is-visible" style="padding: 40px; display: flex; flex-direction: column; height: 100%; border-radius: 24px; position: relative; overflow: hidden;">
                    <div style="position: absolute; top: 30px; right: 30px; opacity: 0.1; color: var(--nk-blue);">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                    </div>
                    <div class="news-meta" style="color: var(--nk-red); font-size: 11px; font-weight: 800; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 2px; font-family: var(--font-display);">
                        03 ÐÐ¿Ñ€ÐµÐ»Ñ, 2024 â€¢ ÐŸÑ€Ð°Ð²Ð¾Ð²Ð¾Ð¹ Ð°ÑƒÐ´Ð¸Ñ‚
                    </div>
                    <h3 style="font-size: 1.5rem; margin-bottom: 20px; font-weight: 800; line-height: 1.3;">Ð˜Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸Ñ Ð² Ñ€Ð°ÑÑ‡ÐµÑ‚Ðµ Ð½Ð°Ð»Ð¾Ð³Ð° Ð½Ð° Ð¸Ð¼ÑƒÑ‰ÐµÑÑ‚Ð²Ð¾ Ð¸ Ð·ÐµÐ¼Ð»ÑŽ</h3>
                    <p style="color: var(--nk-gray-600); font-size: 1rem; line-height: 1.7; flex-grow: 1; margin-bottom: 30px;">
                        Ð¡Ð¾Ð³Ð»Ð°ÑÐ½Ð¾ ÑÑ‚. 175 Ð¸ ÑÑ‚. 402 ÐÐ°Ð»Ð¾Ð³Ð¾Ð²Ð¾Ð³Ð¾ ÐšÐ¾Ð´ÐµÐºÑÐ°, Ð½Ð°Ð»Ð¾Ð³Ð¸ Ð½Ð°Ð¿Ñ€ÑÐ¼ÑƒÑŽ Ð·Ð°Ð²Ð¸ÑÑÑ‚ Ð¾Ñ‚ ÐºÐ°Ð´Ð°ÑÑ‚Ñ€Ð¾Ð²Ð¾Ð¹ ÑÑ‚Ð¾Ð¸Ð¼Ð¾ÑÑ‚Ð¸. Ð Ð°Ð·Ð±Ð¸Ñ€Ð°ÐµÐ¼, ÐºÐ°Ðº Ð¾ÑÐ¿Ð¾Ñ€Ð¸Ñ‚ÑŒ Ð½ÐµÐ²ÐµÑ€Ð½ÑƒÑŽ Ð¾Ñ†ÐµÐ½ÐºÑƒ Ð¸ ÑÐ½Ð¸Ð·Ð¸Ñ‚ÑŒ Ð½Ð°Ð³Ñ€ÑƒÐ·ÐºÑƒ...
                    </p>
                    <div style="margin-top: auto;">
                        <a href="#" class="btn btn--primary btn--small" style="padding: 12px 24px; font-size: 12px;">Ð§Ð¸Ñ‚Ð°Ñ‚ÑŒ Ð¿Ð¾Ð»Ð½Ð¾ÑÑ‚ÑŒÑŽ</a>
                    </div>
                </article>

                <!-- Item 2: Business/Liquidation -->
                <article class="service-card service-card--alt fade-up is-visible fade-up-delay-1" style="padding: 40px; display: flex; flex-direction: column; height: 100%; border-radius: 24px; position: relative; overflow: hidden;">
                    <div style="position: absolute; top: 30px; right: 30px; opacity: 0.2; color: var(--nk-white);">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>
                    </div>
                    <div class="news-meta" style="color: var(--nk-white); opacity: 0.8; font-size: 11px; font-weight: 800; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 2px; font-family: var(--font-display);">
                        28 ÐœÐ°Ñ€Ñ‚Ð°, 2024 â€¢ ÐšÐµÐ¹ÑÐ¸Ð½Ð³
                    </div>
                    <h3 style="font-size: 1.5rem; margin-bottom: 20px; font-weight: 800; line-height: 1.3;">ÐŸÑ€Ð¾Ñ†ÐµÐ´ÑƒÑ€Ñ‹ Ð±Ð°Ð½ÐºÑ€Ð¾Ñ‚ÑÑ‚Ð²Ð° Ð¸ Ð»Ð¸ÐºÐ²Ð¸Ð´Ð°Ñ†Ð¸Ð¸ Ð¿Ñ€ÐµÐ´Ð¿Ñ€Ð¸ÑÑ‚Ð¸Ð¹</h3>
                    <p style="color: rgba(255,255,255,0.7); font-size: 1rem; line-height: 1.7; flex-grow: 1; margin-bottom: 30px;">
                        ÐšÐ¾Ð»Ð»ÐµÐºÑ‚Ð¸Ð² Neksoz Ð¾Ð±ÑŠÐµÐ´Ð¸Ð½ÑÐµÑ‚ ÑÐºÑÐ¿ÐµÑ€Ñ‚Ð¾Ð² Ð² Ð¾Ð±Ð»Ð°ÑÑ‚Ð¸ Ð¿Ñ€Ð°Ð²Ð°. ÐÐ°Ñˆ Ð¾Ð¿Ñ‹Ñ‚ Ð¿Ð¾ÐºÐ°Ð·Ñ‹Ð²Ð°ÐµÑ‚, Ñ‡Ñ‚Ð¾ Ð³Ñ€Ð°Ð¼Ð¾Ñ‚Ð½Ð°Ñ Ð¿Ð¾Ð´Ð³Ð¾Ñ‚Ð¾Ð²ÐºÐ° Ð´Ð¾ÐºÑƒÐ¼ÐµÐ½Ñ‚Ð¾Ð² Ð½Ð° ÑÑ‚Ð°Ñ€Ñ‚Ðµ Ð»Ð¸ÐºÐ²Ð¸Ð´Ð°Ñ†Ð¸Ð¸ ÑÐºÐ¾Ð½Ð¾Ð¼Ð¸Ñ‚ Ð¼ÐµÑÑÑ†Ñ‹ Ñ€Ð°Ð±Ð¾Ñ‚Ñ‹...
                    </p>
                    <div style="margin-top: auto;">
                        <a href="#" class="btn btn--outline-light btn--small" style="padding: 12px 24px; font-size: 12px;">Ð§Ð¸Ñ‚Ð°Ñ‚ÑŒ Ð¿Ð¾Ð»Ð½Ð¾ÑÑ‚ÑŒÑŽ</a>
                    </div>
                </article>

                <!-- Item 3: Forum/Globe -->
                <article class="service-card fade-up is-visible fade-up-delay-2" style="padding: 40px; display: flex; flex-direction: column; height: 100%; border-radius: 24px; position: relative; overflow: hidden;">
                    <div style="position: absolute; top: 30px; right: 30px; opacity: 0.1; color: var(--nk-blue);">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
                    </div>
                    <div class="news-meta" style="color: var(--nk-red); font-size: 11px; font-weight: 800; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 2px; font-family: var(--font-display);">
                        15 ÐœÐ°Ñ€Ñ‚Ð°, 2024 â€¢ ÐœÐµÑ€Ð¾Ð¿Ñ€Ð¸ÑÑ‚Ð¸Ñ
                    </div>
                    <h3 style="font-size: 1.5rem; margin-bottom: 20px; font-weight: 800; line-height: 1.3;">Ð˜Ñ‚Ð¾Ð³Ð¸ ÑÐºÐ¾Ð½Ð¾Ð¼Ð¸Ñ‡ÐµÑÐºÐ¾Ð³Ð¾ Ñ„Ð¾Ñ€ÑƒÐ¼Ð° Ð² Ð”ÑƒÑˆÐ°Ð½Ð±Ðµ</h3>
                    <p style="color: var(--nk-gray-600); font-size: 1rem; line-height: 1.7; flex-grow: 1; margin-bottom: 30px;">
                        ÐžÐ±ÑÑƒÐ¶Ð´Ð°ÐµÐ¼ Ð½Ð¾Ð²Ñ‹Ðµ Ð²ÐµÐºÑ‚Ð¾Ñ€Ñ‹ Ñ€Ð°Ð·Ð²Ð¸Ñ‚Ð¸Ñ Ð¸Ð½Ð²ÐµÑÑ‚Ð¸Ñ†Ð¸Ð¾Ð½Ð½Ð¾Ð³Ð¾ ÐºÐ»Ð¸Ð¼Ð°Ñ‚Ð°. ÐÐ°ÑˆÐ° Ð´ÐµÑÑ‚ÐµÐ»ÑŒÐ½Ð¾ÑÑ‚ÑŒ Ð² Ð¸Ð½Ñ‚ÐµÐ»Ð»ÐµÐºÑ‚ÑƒÐ°Ð»ÑŒÐ½Ð¾Ð¹ ÑÑ„ÐµÑ€Ðµ Ñ‚Ñ€ÐµÐ±ÑƒÐµÑ‚ Ð¿Ð¾ÑÑ‚Ð¾ÑÐ½Ð½Ð¾Ð¹ ÑÐ¸Ð½Ñ…Ñ€Ð¾Ð½Ð¸Ð·Ð°Ñ†Ð¸Ð¸ Ñ Ñ€Ñ‹Ð½ÐºÐ¾Ð¼...
                    </p>
                    <div style="margin-top: auto;">
                        <a href="#" class="btn btn--primary btn--small" style="padding: 12px 24px; font-size: 12px;">Ð§Ð¸Ñ‚Ð°Ñ‚ÑŒ Ð¿Ð¾Ð»Ð½Ð¾ÑÑ‚ÑŒÑŽ</a>
                    </div>
                </article>

            </div>
        </div>
    </div>

            </div>
        </div>
    </div>

</main>

<?php get_footer(); ?>

