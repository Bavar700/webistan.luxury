		...
		if ( $create_if_not_exists ) {
			foreach ( $templates as $location => $template_id ) {
				...
				if ( $location === 'form' ) {
					...
					$post_type_label = method_exists( $this->post_type, 'get_label' )
						? $this->post_type->get_label()
						: ucfirst( $this->post_type->get_key() );
					$location_label = [
						'single' => 'Single post',
						'card' => 'Preview card',
						'archive' => 'Archive',
					][ $location ] ?? ucfirst( str_replace( '_', ' ', $location ) );
					$title = sprintf( '%s: %s', $post_type_label, $location_label );
					$template_type = 'page';
					if ( $location === 'archive' ) {
						$template_type = 'archive';
					} elseif ( $location === 'single' ) {
						$template_type = 'single-post';
					} elseif ( $location === 'card' ) {
						$template_type = 'card';
					}
					$new_template_id = \Voxel\create_template( $title, $template_type );
					if ( ! is_wp_error( $new_template_id ) ) {
						$templates[ $location ] = absint( $new_template_id );
					}
				}
